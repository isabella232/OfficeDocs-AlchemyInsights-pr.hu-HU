---
title: '8000019: Névjegyek hozzáadása és importálása'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999782"
---
# <a name="import-contacts"></a>Névjegyek importálása

Mielőtt névjegyeket importál a Webes Outlook, exportálja őket a másik e-mail-szolgáltatótól .csv fájlba. Ezután kövesse az alábbi utasításokat.

1. A lap alján válassza a <img src='data:image/png;base64,' />
 **Személyek .**
2. Az eszköztáron válassza a **Névjegyek**  >  **importálásának kezelése lehetőséget.**
3. Válassza **a Tallózás** gombot, válassza ki a CSV-fájlt, majd válassza a Megnyitás **gombot.**
4. Válassza az **Importálás lehetőséget.**

További információt a Névjegyek [importálása](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web)a Outlook.

